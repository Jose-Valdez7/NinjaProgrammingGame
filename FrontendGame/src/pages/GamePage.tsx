...
export default function GamePage() {
  ...
  useEffect(() => {
    if (currentLevel >= 2 && !currentUser) {
      // Redirect to login for levels 2+
      window.location.href = '/login'
    }
  }, [currentLevel, currentUser])

  const loadLevel = (levelNumber: number) => {
    const level = levelGenerator.generateLevel(levelNumber)
    if (gameEngineRef.current) {
  ...
  const executeCommands = async () => {
    if (!gameEngineRef.current || !commands.trim()) return

    const validation = commandParser.validateCommands(commands)
    if (!validation.isValid) {
      setError(validation.error || 'Comandos inválidos')
      return
    }
    ...
    try {
      ...
      const expandedCommands = commandParser.expandCommands(parsedCommands)
      
      let currentPos = { x: 0, y: gameState.currentLevel === 1 ? 14 : 14 }
      ...
      for (const command of expandedCommands) {
        for (let step = 0; step < command.steps; step++) {
          ...
          // Check cell type
          const level = levelGenerator.generateLevel(currentLevel)
      ...
      setError('Error ejecutando comandos')
      setIsPlaying(false)
    }
  }
          ...
          <div className="space-y-6">
            ...
            <div className="bg-ninja-purple rounded-lg p-4 border border-blue-500/30">
              ...
              <div className="space-y-2 text-sm">
                ...
                {levelInfo.timeLimit && (
                  <div>⏱️ Tiempo límite: {levelInfo.timeLimit}s</div>
                )}
                {levelInfo.hasGuideLines && (
                  <div className="text-yellow-400">💡 Líneas guía disponibles</div>
                )}
                {levelInfo.allowsLoops && (
                  <div className="text-purple-400">🔄 Loops permitidos</div>
                )}
              </div>
            </div>
